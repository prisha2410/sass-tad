#!/usr/bin/env python3
"""Ablation: no curriculum — train fusion+BiLSTM on Hard only from scratch."""
...
def train(args):
    ...
    print(f"Ablation: NO CURRICULUM (Hard only) | Device: {DEVICE}")
    ...
    train_items, val_items_by_key = [], {}
    for year in YEARS:
        tr = load_cached(feat_dir, year, "hard", "train")
        vl = load_cached(feat_dir, year, "hard", "val")
        if not tr: continue
        train_items.extend(tr)
        val_items_by_key[f"{year}_hard"] = vl
        print(f"  [{year}/hard] train={len(tr)}, val={len(vl)}")

    if len(train_items) == 0:
        print("\nERROR: No training data found. Check --feat-dir.")
        return

    print(f"Total train docs (Hard only): {len(train_items)}")

    ds = FlatDocDataset(train_items)
    loader = DataLoader(ds, batch_size=1, shuffle=True, collate_fn=lambda b: b, num_workers=0)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        avg_hard_f1 = np.mean(list(results.values())) if results else 0.0
...
if __name__ == "__main__":
    ...
    p.add_argument("--base-checkpoint", default="checkpoints/curriculum_228_best.pt")
    p.add_argument("--out-dir", default="checkpoints/ablation_no_curriculum")
