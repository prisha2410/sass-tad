#!/usr/bin/env python3
"""Ablation: no BiLSTM (fusion -> linear projection -> head, no sequence modeling)."""
...
def forward_doc_no_bilstm(model, proj, cls_block, style_block):
    """Fusion -> linear projection (replaces BiLSTM's role) -> head. No sequence modeling."""
    pair_reprs = []
    ...
    stacked = torch.cat(pair_reprs, dim=0)   # (N, 961)
    projected = proj(stacked)                 # (N, 256)
    logits = model.head(projected)             # (N, 1)
...
def evaluate(model, proj, val_items_by_key, threshold):
    model.eval()
    proj.eval()
    results = {}
    with torch.no_grad():
        for key, items in val_items_by_key.items():
            ...
            for cls_np, style_np, lbl_np in items:
                logits = forward_doc_no_bilstm(model, proj, torch.tensor(cls_np), torch.tensor(style_np))
...
def train(args):
    ...
    print(f"Ablation: NO BiLSTM | Device: {DEVICE}")
    ...
    # Fresh linear projection replacing BiLSTM's dimensionality role — NOT loaded from
    # checkpoint (didn't exist before), trained from scratch during this ablation run.
    ...
    weights_map = {"easy": 1.0, "medium": 1.0, "hard": 3.0}
    ...
    if len(flat_items) == 0:
        print("\nERROR: No training data found. Check --feat-dir / --seed.")
        return

    print(f"Total train docs: {len(flat_items)}")

    ds = FlatDocDataset(flat_items)
    sampler = WeightedRandomSampler(flat_weights, len(flat_weights), replacement=True)
    loader  = DataLoader(ds, batch_size=1, sampler=sampler, collate_fn=lambda b: b, num_workers=0)

    trainable_params = [p for p in model.parameters() if p.requires_grad] + list(proj.parameters())
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in tqdm(loader, desc=f"Epoch {epoch}", ncols=80):
            ...
            with autocast(device_type="cuda"):
                logits = forward_doc_no_bilstm(model, proj, cls_t, style_t)
        ...
        results = evaluate(model, proj, val_items_by_key, args.threshold)
        ...
        if avg_hard_f1 > best_hard_f1:
            ...
            torch.save({"model": model.state_dict(), "proj": proj.state_dict()}, out_dir / "best.pt")
            print(f"  ** New best!")
...
if __name__ == "__main__":
    ...
    p.add_argument("--out-dir", default="checkpoints/ablation_no_bilstm")
